...
cur.execute(''' CREATE TABLE IF NOT EXISTS snakestat(
                    ...
                    max_level INT)
            ''')
# function return stat(tuple) if user exists, otherwise return False
def exist(username):
    cur.execute(f"SELECT * FROM snakestat WHERE username = '{username}'")
    row = cur.fetchone() # row=none if not exist
    if row:
        return row
    else: 
        return False
# login by entering username, if user does not exist -> add new user, otherwise show stats of user
def authorization():
    global done, username, userstat
    username = input("enter username: ")
    if not exist(username):
        cur.execute(f"INSERT INTO snakestat (username, last_score, high_score, max_level) VALUES ('{username}', 0, 0, 0)")
        conn.commit()
        print("added new user")
        userstat = exist(username)
        mode = input("enter 'go' to start game: ")
        if mode == '\ngo':
            done = False
    else:
        userstat = exist(username)
        print("info:",f"id: {userstat[0]}", f"username: {userstat[1]}", f"last_score: {userstat[2]}", f"high_score: {userstat[3]}", f"max_level: {userstat[4]}", sep='\n')
        mode = input("enter 'go' to start game: ")
        if mode == '\ngo':
            done = False
# update table
def update_stats(username, last_score, high_score, max_level):
    cur.execute('''UPDATE snakestat 
                    SET last_score = %s, high_score = %s, max_level = %s
                    WHERE username = %s''', (last_score, high_score, max_level, username))
    conn.commit()

# calling function before game scene
authorization()
...
while not done:
    ...
    if snake.dead and not snake.restart:
        update_stats(username, score, max(score, userstat[3]), max(eaten_fruits // 2, userstat[4])) # update stats of user after die
        screen.fill("black") 
